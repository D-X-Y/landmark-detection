...
#
#!/usr/bin/env sh
# bash ./scripts/X-300W/REG-300W-SBT-PF-P68.sh 0 W01 16
echo script name: $0
echo $# arguments
if [ "$#" -ne 3 ] ;then
...
v_batch_size=0
m_batch_size=$3
save_path=./snapshots/X-300W/REG-SBT-PF-${opt}.${stm_w}-${det}-96x96-${i_batch_size}.${v_batch_size}.${m_batch_size}-P68
...
CUDA_VISIBLE_DEVICES=${gpus} python ./exps/STM-main.py \
    ...
    --stm_config   ./configs/X-300W/SBT.REG.PF.${stm_w}.config \
