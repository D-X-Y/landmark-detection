...
#
#!/usr/bin/env sh
# bash ./scripts/300W/REG/OK-STM-REG-PFace.sh 0 W10 16
echo script name: $0
echo $# arguments
if [ "$#" -ne 3 ] ;then
  echo "Input illegal number of parameters " $#
  echo "Need 3 parameters for gpu devices, and the stm-weight, and multi-view batch"
  exit 1
fi
gpus=$1
det=default
...
v_batch_size=0
m_batch_size=$3
save_path=./snapshots/STM-DEMO-300W.PFace.${stm_w}-${det}-REG-96x96-${i_batch_size}.${v_batch_size}.${m_batch_size}

rm -rf ${save_path}

CUDA_VISIBLE_DEVICES=${gpus} python ./exps/STM-main.py \
    --train_lists ./cache_data/lists/300W/300w.train.pth \
		  ./cache_data/lists/Panoptic-FACE/all-face-2000-nopts-novid.pth \
    ...
    --num_pts 68 --data_indicator face49-DEMO-STM --x68to49 \
    --procedure regression \
    --model_config ./configs/face/REG/REG.300W.config \
    --opt_config   ./configs/face/REG/ADAM.L1.300W-REG-Pface.config \
    --stm_config   ./configs/face/REG/STM.REG.300W-PFace-NOV.${stm_w}.config \
