...
#
#!/usr/bin/env sh
# bash ./scripts/300W/REG/OK-SBR-REG-300VW.sh 0 W01 32 32
echo script name: $0
echo $# arguments
if [ "$#" -ne 4 ] ;then
  echo "Input illegal number of parameters " $#
  echo "Need 4 parameters for gpu devices, weights, image-batch-size, and video-batch-size"
  exit 1
fi
gpus=$1
opt=L1
...
save_path=./snapshots/SBR-DEMO-300WVW-${opt}.${sbr_weight}-${det}-REG-96x96-${i_batch_size}.${v_batch_size}
