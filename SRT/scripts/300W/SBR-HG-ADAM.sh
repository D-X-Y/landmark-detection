...
#
#!/usr/bin/env sh
# sh scripts/300W/SBR-HG-ADAM.sh 2 V1 V2 V1 DET 3
echo script name: $0
echo $# arguments
if [ "$#" -ne 6 ] ;then
...
CUDA_VISIBLE_DEVICES=${gpus} python ./exps/SBR-main.py \
    ...
    --save_path    ./snapshots/SBR-300W-${det}-HG-${HGV}-ADAM-${OPTV}-S${sigma}-120x96-${use_gray} \
    --init_model   ./snapshots/PRO-300W-${det}-HG-${HGV}-ADAM-${OPTV}-S${sigma}-120x96-${use_gray}/last-info.pth \
    --pre_crop_expand 0.2 \
