...
#
#!/usr/bin/env sh
# bash scripts/300W/PRO-CPM-SGD.sh 2 V1 V1 DET 3 0
echo script name: $0
echo $# arguments
if [ "$#" -ne 6 ] ;then
...
CUDA_VISIBLE_DEVICES=${gpus} python ./exps/BASE-main.py \
    ...
    --save_path    ./snapshots/PRO-300W-${det}-CPM-${CPMV}-SGD-${OPTV}-S${sigma}-128x96-${use_gray} \
    --pre_crop_expand 0.2 \
    --scale_prob 1.0 --scale_min 0.9 --scale_max 1.1 \
    --offset_max 0.3 --rotate_max 30 \
    --robust_iter 2 \
    --height 128 --width 96 --sigma ${sigma} --batch_size ${batch_size} \
