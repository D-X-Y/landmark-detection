...
#
#!/usr/bin/env sh
# bash ./scripts/300W-V2/SBR-HG-300VW-49.sh 0 W05
echo script name: $0
echo $# arguments
if [ "$#" -ne 2 ] ;then
  echo "Input illegal number of parameters :" $#
  echo "Need 2 parameters for the gpu device and the loss weight"
  exit 1
fi
gpus=$1
opt=ADAM
...
save_path=./snapshots/300W/HEAT-SBR-${sbr_loss}-300WVW-${opt}.${sbr_weight}-${det}-256x256-${i_batch_size}.${v_batch_size}-${point}
...
CUDA_VISIBLE_DEVICES=${gpus} python ./exps/SBR-main.py \
    ...
    --sbr_config   ./configs/300W-V2/SBR-${sbr_loss}.300WVW.${sbr_weight}.config \
    --init_model   ./snapshots/300W/HEAT-DET-300W-RD02-${det}-256x256-S3-L49/last-info.pth \
    --save_path ${save_path} \
    --pre_crop_expand 0.2 \
