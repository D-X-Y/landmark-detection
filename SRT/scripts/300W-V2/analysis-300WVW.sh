...
if [ "$#" -ne 1 ] ;then
  ...
  echo "Need 1 parameters for the-gpu-device"
  exit 1
fi
gpus=$1
opt=ADAM
...
save_path=./snapshots/DEBUG-DXY/HEAT-SBR-${sbr_loss}-300WVW-ANALYSIS

rm -rf ${save_path}

CUDA_VISIBLE_DEVICES=${gpus} python ./exps/analysis-300VW.py \
