...
#
#!/usr/bin/env sh
# bash scripts/WFLW/SRT-REG-PFACE.sh 0
set -e
echo script name: $0
echo $# arguments
if [ "$#" -ne 1 ] ;then
...
save_path=./snapshots/WFLW/REG-SRT-WFLW-PFACE-96x96-${i_batch_size}.${v_batch_size}.${m_batch_size}
rm -rf ${save_path}
...
CUDA_VISIBLE_DEVICES=${gpus} python ./exps/STM-main.py \
    ...
    --opt_config   ./configs/WFLW/ADAM.REG-SRT-PFACE.config \
    --stm_config   ./configs/face/REG/STM.REG.WFLW-PFace.W10.W10.config \
