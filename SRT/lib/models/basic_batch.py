...
def find_tensor_peak_batch(heatmap, radius, downsample, threshold = 0.000001):
  ...
  affine_parameter = torch.zeros((num_pts, 2, 3), device=heatmap.device)
  ...
  grid = F.affine_grid(affine_parameter, grid_size, align_corners=True)
  #sub_feature = F.grid_sample(heatmap.unsqueeze(1), grid, mode='bilinear', padding_mode='reflection', align_corners=True).squeeze(1)
  sub_feature = F.grid_sample(heatmap.unsqueeze(1), grid, mode='bilinear', padding_mode='zeros', align_corners=True).squeeze(1)
  sub_feature = F.threshold(sub_feature, threshold, 0)

  X = torch.arange(-radius, radius+0.0001, radius*1.0/Iradius, device=heatmap.device).view(1, 1, Iradius*2+1)
  Y = torch.arange(-radius, radius+0.0001, radius*1.0/Iradius, device=heatmap.device).view(1, Iradius*2+1, 1)
