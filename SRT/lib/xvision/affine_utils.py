...
def solve2theta(source, target):
  ...
  #X_, qr = torch.gels(source, target) # removed in PyTorch 1.2.0
  X_, qr = torch.lstsq(source, target)
...
def affine2image(image, theta, shape):
  ...
  affI  = F.grid_sample(image.unsqueeze(0), grid, mode='bilinear', padding_mode='border', align_corners=True)
