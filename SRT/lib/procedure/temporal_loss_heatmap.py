...
def calculate_temporal_loss(criterion, heatmaps, locs, past2now, future2now, FBcheck, mask, config):
  ...
  with torch.no_grad():
    ...
    data_ok  = (inmap_ok.view(batch, 1, num_pts, 1).int() + check_ok.view(batch, 1, num_pts, 1).int() + distc_ok.view(batch, 1, num_pts, 1).int() + mask.view(batch, 1, num_pts, 1).int()) == 4

  if config.sbr_loss_type == 'L1':
    past_loss     = criterion.loss_l1_func(locs[:,config.video_L], past2now[:,config.video_L-1], reduction='none')
    future_loss   = criterion.loss_l1_func(locs[:,config.video_L], future2now[:,config.video_L], reduction='none')
    ...
    past_loss     = criterion.loss_mse_func(locs[:,config.video_L], past2now[:,config.video_L-1], reduction='none')
    future_loss   = criterion.loss_mse_func(locs[:,config.video_L], future2now[:,config.video_L], reduction='none')
    temporal_loss = past_loss + future_loss
    final_loss    = torch.masked_select(temporal_loss, data_ok)
    final_loss    = torch.mean( final_loss )
    loss_string   = ''
    ...
    # PAST
    past2now_grid      = identity_grid - (past2now[:,config.video_L-1]-locs[:,config.video_L-1]).view(batch*num_pts, 1, 1, 2)
    past2now_heatmaps  = [x[:,config.video_L-1].contiguous().view(batch*num_pts, 1, H, W) for x in heatmaps]
    past2now_predicts  = [F.grid_sample(x, past2now_grid, mode='bilinear', align_corners=True).view(batch, num_pts, H, W) for x in past2now_heatmaps]
    ...
    # FUTURE
    futu2now_grid      = identity_grid - (future2now[:,config.video_L]-locs[:,config.video_L+1]).view(batch*num_pts, 1, 1, 2)
    futu2now_heatmaps  = [x[:,config.video_L+1].contiguous().view(batch*num_pts, 1, H, W) for x in heatmaps]
    futu2now_predicts  = [F.grid_sample(x, futu2now_grid, mode='bilinear', align_corners=True).view(batch, num_pts, H, W) for x in futu2now_heatmaps]
    ...
    heatmaps_targets   = [x[:,config.video_L].contiguous() for x in heatmaps]

    data_ok            = data_ok.view(batch, num_pts, 1, 1)
    loss_list, loss_string = [], ''
    for index in range( len(heatmaps_targets) ):
      past_loss  = criterion(past2now_predicts[index], heatmaps_targets[index], data_ok)
      futu_loss  = criterion(futu2now_predicts[index], heatmaps_targets[index], data_ok)
