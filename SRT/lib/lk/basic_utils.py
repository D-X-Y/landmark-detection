...
def torch_inverse(deltp):
  ...
  divide = a*d-b*c
  inverse = torch.stack([d, -b, -c, a]).view(2,2)
...
def ComputeGradient(feature, tag):
  ...
  sobel = SobelConv(tag, feature.dtype)
...
def warp_feature(feature, pts_location, patch_size):
  ...
  affine_parameter = [(crop_box[2]-crop_box[0])/2, torch.tensor(0.), (crop_box[0]+crop_box[2])/2,
                      torch.tensor(0.), (crop_box[3]-crop_box[1])/2, (crop_box[1]+crop_box[3])/2]
  affine_parameter = torch.stack(affine_parameter).view(2, 3)
  ...
  sub_feature = F.grid_sample(feature, grid, align_corners=True).squeeze(0)
  return sub_feature
