...
class ToPILImageWithNorm(object):
  ...
  def __call__(self, img):
    assert img.size(0) == 3, 'The image size is not right {:}'.format(img.size())
    assert len(list(img.size())) == 3, 'The image size is not right {:}'.format(img.size())
    if self.normalize is None: return self.topil(img)
    ...
    with torch.no_grad():
      for t, m, s in zip(img, self.normalize.mean, self.normalize.std):
        ...
        t = torch.add(t, m)
        xinput.append( t )
      xinput = torch.stack(xinput).cpu()
    return self.topil(xinput)

  def __repr__(self):
    return self.__class__.__name__ + '(norm={:})'.format(self.normalize)
