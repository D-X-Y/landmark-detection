...
class GeneralDataset(data.Dataset):
  ...
  def _process_(self, image, target, index):
    ...
    if isinstance(image, list):
      if isinstance(image[0], Image.Image)        : height, width = image[0].size[1], image[0].size[0]
      elif isinstance(image[0], torch.FloatTensor): height, width = image[0].size(1), image[0].size(2)
      else: raise Exception('Unknown type of image : {:}'.format( type(image) ))
