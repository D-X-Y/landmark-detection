...
def main(args):
  ...
  for epoch in range(start_epoch, args.epochs):
    ...
    logger.log('==>>{:s} [{:s}], [{:s}], Gradient-Penalty={:}, WGAN-Clip={:}, Critic-Iters={:}, Amsgrad={:}'.format(time_string(), epoch_str, need_time, args.gradient_penalty, args.wgan_clip, args.critic_iters, args.amsgrad))

    #Dloss, Gloss = style_train_plain(args, train_loader, netG, netD, optimizerG, optimizerD, epoch_str, logger)
    ...
    for _iter, ((PlainImages, StyleImages), target, mask, points, image_index, nopoints, cropped_size) in enumerate(train_loader):
      ...
      if args.gradient_penalty: # WGAN-GP
        alpha = torch.rand((Isize[0], 1, 1, 1)).cuda()
        x_hat = alpha * StyleImages.data + (1 - alpha) * G_.data
        x_hat.requires_grad_(True)
        pred_hat = netD(x_hat)
        gradients = torch.autograd.grad(outputs=pred_hat, inputs=x_hat, grad_outputs=torch.ones(pred_hat.size()).cuda(),
                         create_graph=True, retain_graph=True, only_inputs=True)[0]
        gradient_penalty = args.gp_lambda * ((gradients.view(gradients.size()[0], -1).norm(2, 1) - 1) ** 2).mean()
      ...
      if _iter % args.print_freq == 0:
        logger.log(time_string() + ' [Train-WGAN]: [{:}][{:03d}/{:03d}] Gloss {Gloss.val:7.4f} ({Gloss.avg:7.4f}) Dloss {Dloss.val:7.4f} ({Dloss.avg:7.4f})'.format(
